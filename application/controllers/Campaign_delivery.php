...
public function deliver_article_style_js($space_id)
{
//banner + text 
...
if(!empty($hold = $this->campaign_model->get_campaign_by_category_article_style(json_decode($space['category'])[$i],$publisher['country'])
))
{
array_push($space_categories, $hold[mt_rand(0,count($hold)-1)]['category']);
unset($hold);
...
$resulted_campaigns = $this->campaign_model->get_campaign_by_category_article_style($category,$publisher['country']);
//note the singularity
$resulted_campaign = $resulted_campaigns[mt_rand(0,count($resulted_campaigns)-1)];
